...
if "%BOT_CONNECTOR_SETTING%"=="1" ( 
	...
	echo Start bot connector >> %LOGGER_FILE%
    start /B cmd /C call npm start
)
...
if "%CALLFLOW_BOT_SETTING%"=="1" ( 
	...
	echo Start call flow bot >> %LOGGER_FILE%
    start /B cmd /C call npm start 
)
...
if "%PROXY_ADMIN_PAGE_SETTING%"=="1" ( 
    ...
    echo %CURRENT_DIR%\%PROXY_ADMIN_PAGE_DIR%
    cd /d %DESTINATION_DIR%\%PROXY_ADMIN_PAGE_DIR%
    echo Start admin page >> %LOGGER_FILE%
    start /B cmd /C call BackEnd.exe
)
...
if "%PROXY_SERVICE_SETTING%"=="1" ( 
	...
	echo Start proxy service >> %LOGGER_FILE%
    start /B cmd /C call npm start
)
...
if "%INTENT_SVC_SETTING%"=="1" ( 
	...
	echo Start intent service >> %LOGGER_FILE%
    start /B cmd /C call npm start
)

copy %CURRENT_DIR%\my-apache-config.conf %APACHE_CONFIGURE_DIR%
cd /d %APACHE_RUN_BIN_DIR%
...
if %errorlevel% equ 0 (
  echo Apache HTTP Server %serviceName% is already installed.
  echo Apache HTTP Server %serviceName% restart.
  httpd.exe -k restart -n %serviceName%
) else (
  echo Apache HTTP Server %serviceName% is not installed.
  echo Apache HTTP Server %serviceName% install.
  httpd.exe -k install -n %serviceName% -f %APACHE_CONFIGURE_DIR%\my-apache-config.conf
  echo Apache HTTP Server %serviceName% start.
  httpd.exe -k start -n %serviceName%
)

echo Batch script run successfully
endlocal
pause